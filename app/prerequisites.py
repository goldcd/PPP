...
def check_prerequisites():
    ...
    missing_whisper = not is_python_package_installed("whisperx")
    ...
    if not (missing_whisper or missing_ffmpeg or missing_ollama or missing_cuda_support):
        ...
        if not is_ollama_service_running() and not missing_ollama:
            print(f"[OK] whisperx: Found{gpu_status}")
    ...
    if missing_whisper:
        success = install_python_package("whisperx")
        if sys.version_info < (3, 11):
