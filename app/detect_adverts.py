...
def ask_phase1_topics(url, model, blocks_subset, previous_context=None):
    ...
    sys_msg += (
        ...
        "- Hosts will often signpost a sponsor_read in the show with phrases like 'let's take a short break', 'a message from our sponsor', 'we'll be right back', or 'after the break'. The end of such a sponsor_read block may be signposted with similar phrases such as 'Welcome back' or 'back to the show'. IMPORTANT: these signpost phrases are indicators that the content FOLLOWING them is a sponsor_read — the blocks *containing* a signpost phrase are themselves show_content unless a brand pitch also begins in those same blocks. Do not absorb a host's wrap-up or teaser sentence into the adjacent ad segment simply because it contains a transition phrase.\n"
        ...
        "- sponsor_read segments sometimes include a short discussion to make them appear more organic. Include ALL blocks that form part of this organic lead-in WITHIN the sponsor_read — even if the first few blocks sound like casual banter. If the banter is immediately followed by and clearly constructed as a setup for the brand, the entire run (banter + pitch) is one sponsor_read. IMPORTANT: for banter to qualify as an organic lead-in, the brand or product being advertised must be introduced or clearly foreshadowed *within those same banter blocks* — the host is deliberately steering the conversation toward the ad. If the banter covers an independent topic that happens to share a word or theme with the subsequent brand (e.g. hosts discuss a celebrity before an ad that coincidentally uses the same word), it is show_content, not part of the sponsor_read. Only merge banter into an ad segment when it is causally constructed for that brand.\n"
